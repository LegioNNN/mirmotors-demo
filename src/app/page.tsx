...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f9fafb]">
        ...
        <>
          ...
          <section id="arac-listesi" className="mx-auto max-w-7xl px-4 pb-12 pt-10 sm:px-6 lg:px-8">
            ...
            {!loading && (() => {
              ...
              if (filters.brand) tags.push({ label: filters.brand, clear: () => setFilters(f => ({ ...f, brand: "" })) });
              if (filters.segment) {
                const seg = { "Kelepir": "Fırsat", "Yayla Kan": "Ekonomik", "Orta Direk": "Orta", "Premium": "Premium" }[filters.segment] ?? filters.segment;
                tags.push({ label: seg, clear: () => setFilters(f => ({ ...f, segment: "" })) });
              }
              if (filters.priceMin || filters.priceMax) tags.push({ label: `${filters.priceMin ? fmt(filters.priceMin) + " ₺" : "0"} – ${filters.priceMax ? fmt(filters.priceMax) + " ₺" : "∞"}`, clear: () => setFilters(f => ({ ...f, priceMin: "", priceMax: "" })) });
              if (filters.yearMin || filters.yearMax) tags.push({ label: `${filters.yearMin || "?"} – ${filters.yearMax || "?"}`, clear: () => setFilters(f => ({ ...f, yearMin: "", yearMax: "" })) });
              if (filters.kmMin || filters.kmMax) tags.push({ label: `${filters.kmMin ? fmt(filters.kmMin) + " km" : "0"} – ${filters.kmMax ? fmt(filters.kmMax) + " km" : "∞ km"}`, clear: () => setFilters(f => ({ ...f, kmMin: "", kmMax: "" })) });
              if (filters.fuelType) tags.push({ label: filters.fuelType, clear: () => setFilters(f => ({ ...f, fuelType: "" })) });
              if (filters.transmission) tags.push({ label: filters.transmission, clear: () => setFilters(f => ({ ...f, transmission: "" })) });
              if (filters.status) {
                const st = { "Aktif": "Satışta", "Kaporalandı": "Kaporalı", "Satıldı": "Satıldı" }[filters.status] ?? filters.status;
                tags.push({ label: st, clear: () => setFilters(f => ({ ...f, status: "" })) });
              }
              if (tags.length === 0) return null;
              return (
                <div className="mb-4 flex flex-wrap items-center gap-1.5">
                  {tags.map((t) => (
                    <button key={t.label} type="button" onClick={t.clear}
                      className="flex items-center gap-1.5 rounded-full border border-[#111827]/20 bg-[#111827] px-2.5 py-1 text-[11px] font-bold text-white transition-opacity hover:opacity-80">
                      {t.label}
                  ...
                  <button type="button" onClick={() => setFilters({ brand: "", segment: "", priceMin: "", priceMax: "", yearMin: "", yearMax: "", kmMin: "", kmMax: "", status: "", fuelType: "", transmission: "" })}
                    className="rounded-full border border-gray-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-gray-500 hover:bg-gray-50">
                    Tümünü Temizle
                  </button>
                </div>
              );
                ...
                <div className="lg:col-span-8 order-2 lg:order-none space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {brand.social.youtube && (
                      <SocialCard
                        platform="YouTube"
                        handle={brand.social.youtube.displayName}
                        sub={brand.social.youtube.handle}
                        href={brand.social.youtube.url}
                        stats={[{ val: brand.social.youtube.subscribers, lbl: "abone" }]}
                        iconBg="bg-[#FF0000]"
                        icon={
                          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="sm:col-span-2">
              <img src={brand.logos.horizontalDark} alt={brand.name} className="h-20 w-auto" />
      ...
      {/* Mobil WA sticky button */}
      <a
        href={`https://wa.me/${brand.primaryPhone.replace(/^\+/, "")}?text=${encodeURIComponent("Merhaba, araçlarınız hakkında bilgi almak istiyorum.")}`}
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-20 left-4 z-40 flex h-14 w-14 items-center justify-center rounded-2xl shadow-xl transition-transform active:scale-95 lg:hidden"
