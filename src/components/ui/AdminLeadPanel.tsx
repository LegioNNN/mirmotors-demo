      ...
      <div className="divide-y divide-gray-100">
        {leads.map((lead) => {
          ...
          return (
            <div
              key={lead.id}
              className={`px-5 py-4 transition-colors sm:px-6 ${
                lead.status === "Reddedildi" ? "opacity-50" : ""
              }`}
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`inline-flex shrink-0 items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${cfg.bg} ${cfg.text} border-${cfg.dot.replace("bg-", "")}/30`}
                >
              ...
              <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() => {
                        ...
                        const cleaned = lead.phone
                          .replace(/^\+/, "")
                          .replace(/[^0-9]/g, "");
                        if (cleaned) window.open(`tel:${cleaned}`, "_blank");
                      }}
                      className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3.5 py-1.5 text-[11px] font-bold text-white transition-colors hover:bg-emerald-500 active:bg-emerald-700"
                    >
                      KABUL ET / ARA
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const url = generateRedWhatsappUrl(
                          lead.phone,
                          lead.brand,
                          lead.model,
                          lead.year
                      ...
                      className="inline-flex items-center gap-1.5 rounded-lg bg-red-600 px-3.5 py-1.5 text-[11px] font-bold text-white transition-colors hover:bg-red-500 active:bg-red-700"
                    >
                      NAZİKÇE REDDET
                    </button>
                  </div>
                ...
                {lead.status === "Kabul Edildi" && (
                  <span className="text-[11px] text-emerald-600">Müşteri aranacak</span>
                )}
                {lead.status === "Reddedildi" && (
                  <span className="text-[11px] text-red-500">Red mesajı WhatsApp&apos;tan gönderildi</span>
                )}
              </div>
            </div>
          );
