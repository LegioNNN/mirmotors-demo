  ...
  return (
    <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
      ...
      <button
        ...
        className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-left transition-colors hover:bg-amber-100/80"
      >
        ...
        <div>
          <p className="text-xs font-bold text-amber-800 uppercase tracking-wider">Onay Bekleyen</p>
      ...
      <button
        ...
        className={`flex items-center gap-3 rounded-xl border px-4 py-3 text-left transition-colors ${
          todayCount > 0
            ? "border-amber-400 bg-amber-50 hover:bg-amber-100/80"
            : "border-gray-200 bg-gray-50 opacity-60"
        }`}
      >
        <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${
          todayCount > 0 ? "bg-amber-200" : "bg-gray-200"
        }`}>
          <svg className={`h-5 w-5 ${
            todayCount > 0 ? "text-amber-700" : "text-gray-500"
        ...
        <div>
          <p className={`text-xs font-bold uppercase tracking-wider ${
            todayCount > 0 ? "text-amber-800" : "text-gray-500"
          }`}>Bugün Dönüş</p>
          <p className={`text-lg font-black ${
            todayCount > 0 ? "text-amber-900" : "text-gray-500"
          }`}>{todayCount}</p>
        </div>
      </button>
