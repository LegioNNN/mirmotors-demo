...
export default function Navbar({
  ...
  const [mirbotOpen, setMirbotOpen] = useState(false);
    ...
    <nav className="sticky top-0 z-50 bg-white border-b border-gray-200">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          >
            <img src={brand.logos.horizontal} alt={brand.name} className="h-20 w-auto" />
          </button>
          ...
          <div className="hidden items-center gap-3 md:flex">
            ...
            <div className="relative">
              <button
                type="button"
                onClick={() => setMirbotOpen((v) => !v)}
                className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#111827] to-[#1f2937] px-4 py-2 text-sm font-bold text-white shadow-lg transition-all hover:shadow-xl hover:scale-[1.03] border border-white/10"
              >
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#25D366] text-[10px]">
              ...
              {mirbotOpen && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setMirbotOpen(false)} />
                  <div className="absolute right-0 top-12 z-50 w-72 rounded-2xl bg-[#111827] p-5 shadow-2xl border border-white/10 animate-in fade-in slide-in-from-top-2 duration-200">
                    {/* Header */}
                    <div className="flex items-center gap-3 mb-4">
                      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#25D366]/20 border border-[#25D366]/30">
                        <svg viewBox="0 0 24 24" fill="#25D366" className="h-5 w-5">
                          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                        </svg>
                      </div>
                      <div>
                        <p className="text-sm font-black text-white">MirBot</p>
                        <div className="flex items-center gap-1.5">
                          <span className="h-1.5 w-1.5 rounded-full bg-[#25D366] animate-pulse" />
                          <p className="text-[11px] text-[#25D366] font-medium">Çevrimiçi</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <p className="text-xs text-gray-300 leading-relaxed mb-4">
                      WhatsApp üzerinden anlık destek. Araç sorularınızı sorun, fiyat alın, randevu ayarlayın.
                    </p>
                    ...
                    <button
                      type="button"
                      onClick={() => { setMirbotOpen(false); handleWhatsapp(); }}
                      className="w-full rounded-xl bg-[#25D366] py-2.5 text-sm font-bold text-white hover:bg-[#20ba5a] transition-colors"
                    >
                      WhatsApp'ta Başlat
                    </button>
                    <p className="mt-2 text-center text-[10px] text-gray-500">Ortalama yanıt süresi: &lt; 2 dk</p>
                  </div>
                </>
              )}
            </div>
