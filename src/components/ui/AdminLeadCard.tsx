    ...
    <div className={`relative flex border-b border-gray-100 ${lead.status === "Reddedildi" ? "opacity-60" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div
          ...
          onKeyDown={(e) => e.key === "Enter" && onToggle(lead.id)}
          className={`flex w-full cursor-pointer items-center gap-3 px-4 py-3 transition-colors duration-150 select-none sm:px-5 ${open ? "bg-[#111827] hover:bg-[#1a2333]" : "hover:bg-gray-50/60"}`}
        >
          {/* İsim */}
          ...
          {/* Fotoğraf göstergesi */}
          {hasPhotos && (
            <span className="shrink-0 hidden items-center gap-1 text-[10px] font-semibold text-blue-500 sm:flex">
          ...
          {/* Hızlı aksiyon: Ara + WA */}
          {lead.phone && (
            <div className="shrink-0 flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
              <a
                href={`tel:${lead.phone.replace(/^\+/, "").replace(/[^0-9]/g, "")}`}
                title="Ara"
                className={`flex h-7 w-7 items-center justify-center rounded-lg transition-colors ${open ? "bg-white/10 hover:bg-emerald-500 text-white" : "bg-emerald-50 hover:bg-emerald-500 text-emerald-600 hover:text-white"}`}
              >
              ...
              <a
                href={`https://wa.me/${lead.phone.replace(/^\+/, "").replace(/[^0-9]/g, "")}?text=${encodeURIComponent("Merhaba, araç alım talebiniz hakkında görüşmek istiyoruz.")}`}
                target="_blank"
                rel="noopener noreferrer"
                title="WhatsApp"
                className={`flex h-7 w-7 items-center justify-center rounded-lg transition-colors ${open ? "bg-white/10 hover:bg-[#25d366] text-white" : "bg-green-50 hover:bg-[#25d366] text-green-600 hover:text-white"}`}
              >
